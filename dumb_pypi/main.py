...
class Package(NamedTuple):
    ...
    def formatted_upload_time(self) -> str:
        ...
        dt = datetime.fromtimestamp(self.upload_timestamp, tz=UTC)
    ...
    def input_json(self) -> dict[str, Any]:
        ...
        return {
            k: getattr(self, k) for k in inspect.getfullargspec(self.create).kwonlyargs if getattr(self, k) is not None
        }
...
def _package_json(sorted_files: list[Package], base_url: str) -> dict[str, Any]:
    ...
    return {
        ...
        "releases": {version: [file_.json_info(base_url) for file_ in files] for version, files in by_version.items()},
...
def build_repo(
    ...
    current_date = _format_datetime(datetime.now(UTC))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--packages-url",
        help="url to packages (can be absolute or relative)",
        required=True,
    )
    parser.add_argument(
        "--title",
    ...
    parser.add_argument(
        ...
        help=("Don't template creation timestamp in outputs.  This option makes the output repeatable."),
    )
    parser.add_argument(
        "--no-per-release-json",
        action="store_true",
