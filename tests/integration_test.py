...
def test_normalized_packages_wheels(
    ...
    """
    install_packages(tmpweb.path, (FakePackage("aspy.yaml-0.2.1-py2.py3-none-any.whl"),))
    pip_download(
        pip,
        tmpweb.url + "/simple",
        tmpdir.strpath,
...
def test_pip_uses_core_metadata(capfd, tmpdir, tmpweb, pip):
    install_packages(tmpweb.path, (FakePackage("foo-1-py2.py3-none-any.whl"),))
    pip_download(
        pip,
        tmpweb.url + "/simple",
        tmpdir.strpath,
    ...
    assert f"Downloading {tmpweb.url}/pool/foo-1-py2.py3-none-any.whl.metadata" in capfd.readouterr().out
